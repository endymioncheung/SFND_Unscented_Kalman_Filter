...
{
	// Calculate polar coordinates from vehicle's cartestian coordinate
	...
	// Set radar measurement and time stamp
	MeasurementPackage meas_package;
	meas_package.sensor_type_ = MeasurementPackage::RADAR;
    meas_package.raw_measurements_ = VectorXd(3); // [rho, phi, rho_dot]
...
void Tools::ukfResults(Car car, pcl::visualization::PCLVisualizer::Ptr& viewer, double time, int steps)
{
	...
	if(time > 0)
	{
		...
		{
			ukf.Prediction(dt);
			// Display spheres for prediction of future path
